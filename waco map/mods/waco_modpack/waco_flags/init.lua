...
local DEFAULT_FLAG = minetest.settings:get('default_flag') or 'waco'
...
local delete_if_orphan = function( self )
	...
	if node.name ~= "waco_flags:upper_mast" and node.name ~= "ignore" then
...
minetest.register_entity( "waco_flags:wavingflag", {
	initial_properties = {
		physical = false,
		visual = "mesh",
	...
	on_activate = function ( self, staticdata, dtime )
		...
		for o=1, #objs do
			...
			if lua and self ~= lua and lua.name == "waco_flags:wavingflag" then
...
minetest.register_node("waco_flags:lower_mast", {
...
local function spawn_flag( pos )
	...
	local obj = minetest.add_entity( flag_pos, "waco_flags:wavingflag" )
...
minetest.register_node( "waco_flags:upper_mast", {
	...
	drop = {"waco_flags:lower_mast 2", "waco_flags:upper_mast"},
	...
	on_place = function( itemstack, placer, pointed_thing )
		...
		if (node.name ~= "waco_flags:lower_mast" or node2.name ~= "waco_flags:lower_mast") then
			return itemstack
		end
		...
		minetest.set_node(pointed_thing.under - vector.new(0, 1, 0), {name = "waco_flags:upper_mast", param2 = param2 })
		minetest.set_node(pointed_thing.under, {name = "waco_flags:upper_mast_hidden_1"})

		if not (minetest.is_creative_enabled(pn)) then
			itemstack:take_item()
		end

		local def = minetest.registered_nodes["waco_flags:upper_mast"]
	...
	after_destruct = function( pos )
		...
		if minetest.get_node( above1 ).name == "waco_flags:upper_mast_hidden_1" then
...
minetest.register_lbm({
	name = "waco_flags:respawn_flags",
	label = "Respawn flags",
	nodenames = {"waco_flags:upper_mast"},
...
minetest.register_lbm({
	name = "waco_flags:update_node_meta",
...
if minetest.registered_items[flagpole_material] then
	minetest.register_craft({
		output = "waco_flags:lower_mast 3",
		recipe = {
			{flagpole_material},
			{flagpole_material},
			{flagpole_material},
...
-- or destructed.
minetest.register_node( "waco_flags:upper_mast_hidden_1", {
	drawtype = "airlike",
	pointable = false,
	paramtype = "light",
...
flags.set_flag_at = function( pos, flag_name )
	...
	if node.name ~= "waco_flags:upper_mast" then
		return false
	end
